 ...
 */
 function findExternalOrigin(moduleGraph, issuer) {
  if (!_.isNil(issuer) && _.startsWith(issuer.rawRequest, './')) {
    return findExternalOrigin(moduleGraph, moduleGraph.getIssuer(issuer));
  }
  return issuer;
}
...
function getExternalModules({ compilation }) {
  ...
  for (const module of compilation.modules) {
    if (isExternalModule(module)) {
      externals.add({
        origin: _.get(findExternalOrigin(compilation.moduleGraph, compilation.moduleGraph.getIssuer(module)), 'rawRequest'),
